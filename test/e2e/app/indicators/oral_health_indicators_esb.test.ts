// @filename: oral_health_indicators_esb.test.ts
...
describe('E2E (FHIR): Fichas Técnicas SAPS - Indicadores de Saúde Bucal (B1 a B6) no RJ', () => {
    ...
    it('1. Deve identificar equipes de Saúde Bucal (CareTeam) no CNES do RJ via FHIR', async () => {
        ...
        const esbTeams = store.getCareTeams(team => {
            const code = team.category?.[0]?.coding?.[0]?.code;
            const name = String(team.name || '').toUpperCase();
            return ['71', '16', '17', '18', '19'].includes(code || '') || name.includes('BUCAL') || name.includes('ODONTO') || name.includes('ESB');
        });

        console.log(`\n[FHIR/CNES/RJ] Total de equipes de Saúde Bucal (CareTeam) mapeadas: ${esbTeams.length}`);
        assert.ok(esbTeams.length > 0, `Deve encontrar equipes de Saúde Bucal no RJ (encontradas: ${esbTeams.length})`);
    ...
    it('2. Deve computar os casos de B1 a B6 para as equipes de Saúde Bucal e Estado do RJ', async () => {
        console.log(`\n=== RESUMO E2E FHIR: INDICADORES DE SAÚDE BUCAL (B1 a B6 - RJ) ===`);
        ...
        assert.strictEqual(stateResults.length, 6, 'Todos os 6 indicadores de eSB (B1 a B6) devem ter resultado consolidado');
    });
});
